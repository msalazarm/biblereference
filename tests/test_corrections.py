...
def test_every_reason_written_in_the_file_is_reachable() -> None:
    ...
    # 200 written, 195 distinct: five entries share their prose with a sibling, which is
    # what a fix explained once and applied twice looks like.
    assert len(occurrences) == 200, "the count moved; add or remove the reason deliberately"
    assert len(written) == 195
