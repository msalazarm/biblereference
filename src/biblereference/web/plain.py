"""The server-rendered browsing page: one form, one table, and the texts underneath.

No JavaScript, no assets, one round trip. It is the fallback the richer screens fall back
*to* -- a browser with scripting off, a text browser, a machine reading with ``curl`` -- and
it is also the thing that has to keep working while the rest is built.

The covering toggle is the interesting part: identical for almost every verse, which is the
point, and where it differs is where one edition merges what another divides. Some to try:
...
from ..versification import DEFAULT_SYSTEMS, VersificationError
from .library import VRS, by_system
...
def passage(corpus: SqliteCorpus, span: VerseRange, covering: bool) -> str | None:
    ...
    for segment in segments:
        ...
        # `available`, not `fetch`: a corpus missing any one verse of the range would
        # otherwise show nothing at all, and a partial passage is worth reading. It reads
        # by chapter, so a psalm is one query rather than 176 -- per corpus, per request.
        verses.extend(
